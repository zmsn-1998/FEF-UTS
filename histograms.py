...
for k, name in enumerate(names):
    print(name)
    regret.append((Exp3G_hist[k, 0] / n_runs,
                   1.96 * np.sqrt((Exp3G_hist[k, 1] / n_runs - (Exp3G_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((UCB_N_hist[k, 0] / n_runs,
                   1.96 * np.sqrt((UCB_N_hist[k, 1] / n_runs - (UCB_N_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((UCB_MaxN_hist[k, 0] / n_runs,
                   1.96 * np.sqrt((UCB_MaxN_hist[k, 1] / n_runs - (UCB_MaxN_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((Thompson_N_hist[k, 0] / n_runs,
                   1.96 * np.sqrt((Thompson_N_hist[k, 1] / n_runs - (Thompson_N_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((Thompson_MaxN_hist[k, 0] / n_runs, 1.96 * np.sqrt(
        (Thompson_MaxN_hist[k, 1] / n_runs - (Thompson_MaxN_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((Generalized_UCB_hist[k, 0] / n_runs, 1.96 * np.sqrt(
        (Generalized_UCB_hist[k, 1] / n_runs - (Generalized_UCB_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((Generalized_UCB_inv_hist[k, 0] / n_runs, 1.96 * np.sqrt(
        (Generalized_UCB_inv_hist[k, 1] / n_runs - (Generalized_UCB_inv_hist[k, 0] / n_runs) ** 2) / n_runs)))
    regret.append((Generalized_UCB_exp_hist[k, 0] / n_runs, 1.96 * np.sqrt(
        (Generalized_UCB_exp_hist[k, 1] / n_runs - (Generalized_UCB_exp_hist[k, 0] / n_runs) ** 2) / n_runs)))
for k, name in enumerate(names):
    print(name)
    print("ExpG yields", Exp3G_hist[k, 0] / n_runs, "+/-",
          1.96 * np.sqrt((Exp3G_hist[k, 1] / n_runs - (Exp3G_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("UCB_N yields", UCB_N_hist[k, 0] / n_runs, "+/-",
          1.96 * np.sqrt((UCB_N_hist[k, 1] / n_runs - (UCB_N_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("UCB_MaxN yields", UCB_MaxN_hist[k, 0] / n_runs, "+/-",
          1.96 * np.sqrt((UCB_MaxN_hist[k, 1] / n_runs - (UCB_MaxN_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("Thompson_N yields", Thompson_N_hist[k, 0] / n_runs, "+/-",
          1.96 * np.sqrt((Thompson_N_hist[k, 1] / n_runs - (Thompson_N_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("Thompson_MaxN yields", Thompson_MaxN_hist[k, 0] / n_runs, "+/-",
          1.96 * np.sqrt((Thompson_MaxN_hist[k, 1] / n_runs - (Thompson_MaxN_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("Generalized_UCB yields", Generalized_UCB_hist[k, 0] / n_runs, "+/-",
          1.96 * np.sqrt((Generalized_UCB_hist[k, 1] / n_runs - (Generalized_UCB_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("Generalized_UCB_inv yields", Generalized_UCB_inv_hist[k, 0] / n_runs, "+/-", 1.96 * np.sqrt(
        (Generalized_UCB_inv_hist[k, 1] / n_runs - (Generalized_UCB_inv_hist[k, 0] / n_runs) ** 2) / n_runs))
    print("Generalized_UCB_exp yields", Generalized_UCB_exp_hist[k, 0] / n_runs, "+/-", 1.96 * np.sqrt(
        (Generalized_UCB_exp_hist[k, 1] / n_runs - (Generalized_UCB_exp_hist[k, 0] / n_runs) ** 2) / n_runs))
